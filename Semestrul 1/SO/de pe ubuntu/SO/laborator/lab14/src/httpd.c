...
int main(int argc, char *argv[]) {
    char *service = "12345", *p; // port default daca nu e dat argument
    int sock, fd;
    ...
    for (;;) {
        ...
        while ((n = read(fd, buf, sizeof(buf))) > 0) {
            ...
            // get size
            int fdIndex = open("bosch.html", O_RDWR);
            char bufferIndex[256];
            ssize_t read_bytes;
            int size = 0;
            if (fdIndex < 0) {
                perror("open");
            ...
            while ((read_bytes = read(fdIndex, bufferIndex, sizeof(bufferIndex))) > 0) {
                size += read_bytes;
            }
            ...
            // write
            fdIndex = open("bosch.html", O_RDWR);
            if (fdIndex < 0) {
                perror("open");
                exit(1);
            }
            while ((read_bytes = read(fdIndex, bufferIndex, sizeof(bufferIndex))) > 0) {
                write(fd, bufferIndex, read_bytes);
            }
            close(fdIndex);
        }
        close(fd);
